...
{
	g_hCreditsNormal = CreateConVar("kxnrl_surf_normal", "50", "Credits given when a player finishes a map.");
	g_hCreditsBonus = CreateConVar("kxnrl_surf_bonus", "100", "Credits given when a player finishes a bonus.");
	g_hCreditsPractice = CreateConVar("kxnrl_surf_practice", "25", "Credits given when a player finishes a map in practice mode.");
	g_hCreditsNormalAfterCompletion = CreateConVar("kxnrl_surf_normal_again", "20", "Credits given when a player finishes a map again.");
	g_hCreditsBonusAfterCompletion = CreateConVar("kxnrl_surf_bonus_again", "50", "Credits given when a player finishes a bonus again.");
	g_hCreditsPracticeAfterCompletion = CreateConVar("kxnrl_surf_practice_again", "5", "Credits given when a player finishes a map in practice mode again.");
...
public Action surftimer_OnMapFinished(int client, float fRunTime, char sRunTime[54], int rank, int total)
{
	if(!mapFinished[client])
	{
		...
		Store_SetClientCredits(client, Store_GetClientCredits(client) + g_CreditsNormal, "map finish");
		mapFinished[client] = true;
	}
	else
	{
		CPrintToChat(client, "%t", "OnMapFinishedAgain", g_sTag, g_CreditsNormalAfterCompletion);
		Store_SetClientCredits(client, Store_GetClientCredits(client) + g_CreditsNormalAfterCompletion, "after map finish");
	}
}
...
public Action surftimer_OnBonusFinished(int client, float fRunTime, char sRunTime[54], int rank, int total, int bonusid)
{
	if(!bonusFinished[client])
	{
		...
		Store_SetClientCredits(client, Store_GetClientCredits(client) + g_CreditsBonus, "bonus finish");
		bonusFinished[client] = true;
	}
	else
	{
		CPrintToChat(client, "%t", "OnBonusFinishedAgain", g_sTag, g_CreditsBonusAfterCompletion);
		Store_SetClientCredits(client, Store_GetClientCredits(client) + g_CreditsBonusAfterCompletion, "after bonus finish");
	}
}
...
public Action surftimer_OnPracticeFinished(int client, float fRunTime, char sRunTime[54])
{
	if(!practiceFinished[client])
	{
		...
		Store_SetClientCredits(client, Store_GetClientCredits(client) + g_CreditsPractice, "prac finish");
		practiceFinished[client] = true;
	}
	else
	{
		CPrintToChat(client, "%t", "OnPracticeFinishedAgain", g_sTag, g_CreditsPracticeAfterCompletion);
		Store_SetClientCredits(client, Store_GetClientCredits(client) + g_CreditsPracticeAfterCompletion, "after prac finish");
	}
}
